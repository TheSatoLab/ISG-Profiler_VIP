...
Aves_Mam_mbio_genomad_filt <- read.table("Aves_Mam_mbio_genomad_filt_250630_for168438.txt", sep = "\t", header = T) %>%
  ...
  select(-Virus_g)
# Aves_Mam_mbio_genomad_filt_old <- read.table("Aves_Mam_mbio_genomad_filt_250630.txt", sep = "\t", header = T) %>%
#   separate(taxonomy, into = c("Virus_g", "Realm_g", "Kingdom_g", "Phylum_g", "Class_g", "Order_g", "Family_g"), sep = ";", fill = "right") %>%
#   mutate(across(where(is.character), ~ na_if(., ""))) %>%
#   select(-Virus_g)
...
herpes_nohit_seq <- herpes_nohit %>%
  select(seq_name) %>%
  unique()
#write.table(herpes_nohit_seq, "noherpes.txt", row.names = F, sep = "\t", quote = F)
...
blastx_vir <- read_tsv("blast/viral_hit_55366_250606.blastx_tax.txt") %>%
  separate(tax_lin, into = c("Realm_b", "Kingdom_b", "Phylum_b", "Class_b", "Order_b", "Family_b", "Genus_b", "Species_b"), sep = ";", fill = "right") %>%
  mutate(across(where(is.character), ~ na_if(., ""))) %>%
  group_by(seq_name) %>%
  slice_max(order_by = bitscore, n = 1, with_ties = FALSE) %>%
  ...
  rename(bl_len = length)
blastx_tmp <- blastx %>%
  separate(col = seq_name, into = c("ID", "seq_num"), sep = "_")
...
genomad_blastx_vir <- Aves_Mam_mbio_genomad_filt %>%
  left_join(blastx_vir, by = c("seq_name")) %>%
  mutate(across(where(is.character), ~ na_if(., "")))

herpes_vir <- genomad_blastx_vir %>%
  filter(Family_g == "Orthoherpesviridae")

herpes_nohit_vir <- herpes_vir %>%
  filter(is.na(Acc_ID))
...
plot_family_num <- family_id_count %>%
  ...
  mutate(Family_g = fct_reorder(Family_g, n, .fun = sum))  # 合計値で順序指定
...
#herpes
# herpes <- classified_mod %>% filter(Family_g == "Orthoherpesviridae")
# blastx_ori <- read_tsv("blast/tophit_lin_mod.blastx.txt")

undetected_seq <- blast_labelled %>%
  filter(identity_group == "Undetected") %>%
  select(seq_name) %>%
  unique()

#write.table(undetected_seq, "blastx_undetected.txt", row.names = F, sep = "\t", quote = F)

undetected_herp_seq <- blast_labelled %>%
  filter(Family_g == "Orthoherpesviridae" & identity_group == "Undetected") %>%
  select(seq_name) %>%
  unique()

#write.table(undetected_herp_seq, "blastx_herp_undetected.txt", row.names = F, sep = "\t", quote = F)
...
host_pie_data <- host %>%
  ...
  count(species, sort = TRUE) %>%   # 出現数カウント（多い順）
  mutate(
